...
struct AudioVoice : public juce::SynthesiserVoice
{
        ...
        juce::SynthesiserSound* sound, int currentPitchWheelPosition) override
    {
...
#if _REAL_TIME_
        auto buffer = audio->original;
        double pitch = juce::MidiMessage::getMidiNoteInHertz(midiNoteNumber) / audio->baseFrequency;

        juce::AudioSampleBuffer output(1, buffer.getNumSamples() * buffer.getNumChannels());
        juce::AudioDataConverters::interleaveSamples(buffer.getArrayOfReadPointers(), output.getWritePointer(0), buffer.getNumSamples(), buffer.getNumChannels());

        shifter.init(output.getNumChannels(), sampleRate);
        ...
        processed = buffer;
        juce::AudioDataConverters::deinterleaveSamples(output.getReadPointer(0), processed.getArrayOfWritePointers(), processed.getNumSamples(), processed.getNumChannels());

        processed = SampleUtils::resample(processed, audio->getSampleRate(), sampleRate);
        loop = SampleUtils::resample(buffer, audio->getSampleRate(), sampleRate);
#else
...
private:
    ...
    juce::AudioSampleBuffer processed;
    juce::AudioSampleBuffer loop;
    int sourceIndex = 0;
    int sampleRate = 0;
